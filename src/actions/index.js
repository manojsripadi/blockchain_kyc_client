...
export const loginUser = (userDetails = {}) => async dispatch => {
    const res = await axios.post('/api/account/login', userDetails);
...
export const getUserDetails = (token) => async dispatch => {
    ...
    const res = await axios.get('/ipfs/request', { headers });
...
export const uploadDocuments = (payload) => async dispatch => {
    ...
    const res = await axios.post('/ipfs/upload', formData, { headers });
    dispatch({ type: UPLOAD_DOCUMENTS })
}

export const approveDocuments = (payload) => async dispatch => {
    try {
        const res = await axios.post('/bc/create-ious', payload);
...
export const rejectDocuments = (payload, status) => async dispatch => {
    try {
        const res = await axios.put(`/api/account/status/${status}`, payload);
        if(status === 'REJECTED') {
...
export const getStatus = (token) => async dispacth => {
    ...
    const res = await axios.get('/api/account/fetchStatus', {headers});
